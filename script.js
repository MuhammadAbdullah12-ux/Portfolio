...
// ============================================
// NANOLLM WORKING DETAILS MODAL & PLAYGROUND
// ============================================
(function nanollmModal() {
  const modal = document.getElementById('nanollm-modal');
  if (!modal) return;

  const openBtns = document.querySelectorAll('.open-details-btn');
  const closeBtn = document.getElementById('close-nanollm-modal');
...
(function nanollmPlayground() {
  const selectPrompt = document.getElementById('prompt-select');
  ...
  const termStream = document.getElementById('term-stream-text');

  if (!btnGenerate) return;

  // Live slider value label updates
  ...
  if (sliderTopp && valTopp) {
    sliderTopp.addEventListener('input', () => { valTopp.textContent = parseFloat(sliderTopp.value).toFixed(2); });
  }

  // Pre-configured response banks tailored to prompt & temperature setting
  const responseBank = {
    shakespeare: {
      lowTemp: "JULIET: O gentle Romeo, if thou dost love, pronounce it faithfully. Or if thou think'st I am too quickly won, I'll frown and be perverse and say thee nay, so thou wilt woo.",
      medTemp: "JULIET: Be not dismay'd, fair lord; the moonlight shines upon our vows. For love's light wings did I o'erperch these walls, and soft attention heads compute our destiny.",
      highTemp: "JULIET: Ha! What vector star strikes through yonder embedding window? 'Tis the east, and Juliet is the gradient update! Arise, fair sun, and clip the exploding losses!"
    },
    philosophy: {
      lowTemp: "To understand the nature of intelligence, one must first analyze the mechanisms of pattern recognition and causal inference across layered statistical representations.",
      medTemp: "To understand the nature of intelligence, one must first observe how self-attention transforms discrete symbol sequences into continuous semantic spaces.",
      highTemp: "To understand the nature of intelligence, one must first query the latent void — where loss curves decay like ancient autumn leaves under cosine annealing."
    },
    code: {
      lowTemp: "def train_transformer(model, dataloader, optimizer):\n    model.train()\n    for batch in dataloader:\n        optimizer.zero_grad()\n        loss = model(batch)\n        loss.backward()\n        torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)\n        optimizer.step()",
      medTemp: "def train_transformer(model, dataloader, optimizer):\n    # Optimized NanoLLM training step with cosine decay\n    for x, y in dataloader:\n        logits, loss = model(x, y)\n        loss.backward()\n        optimizer.step()\n        optimizer.zero_grad(set_to_none=True)",
      highTemp: "def train_transformer(model, dataloader, optimizer):\n    # Autoregressive BPE decoding & sampling loop\n    tokens = model.generate(prompt_ids, temp=0.8, top_k=20, top_p=0.9)\n    return tiktoken_decode(tokens)"
    }
  };
  ...
  btnGenerate.addEventListener('click', () => {
    const pKey = selectPrompt.value || 'shakespeare';
    const temp = parseFloat(sliderTemp.value);
    const topk = sliderTopk.value;
    const topp = parseFloat(sliderTopp.value).toFixed(2);

    const promptText = selectPrompt.options[selectPrompt.selectedIndex].text.replace(/"/g, '');

    // Update command line display
    termPrompt.textContent = `$ python generate.py --prompt "${promptText.substring(0, 30)}..." --temp ${temp} --top_k ${topk} --top_p ${topp}`;

    // Choose target text based on temp
    let targetText = '';
    const bank = responseBank[pKey] || responseBank.shakespeare;
    if (temp < 0.5) targetText = bank.lowTemp;
    else if (temp <= 1.0) targetText = bank.medTemp;
    else targetText = bank.highTemp;

    // Animate streaming token generation
    if (typeTimer) clearInterval(typeTimer);
    termStream.textContent = 'Generating tokens...';
    let idx = 0;

    btnGenerate.disabled = true;
    btnGenerate.innerHTML = '<span>Sampling... ⏳</span>';
    ...
    typeTimer = setInterval(() => {
      ...
      if (idx >= targetText.length) {
        ...
        btnGenerate.innerHTML = '<span>Generate Tokens ⚡</span>';
      }
      termStream.textContent = targetText.substring(0, idx);
    }, 25);
  });
})();
